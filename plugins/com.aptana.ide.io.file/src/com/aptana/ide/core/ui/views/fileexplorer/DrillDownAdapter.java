...
/**
 * Implements a simple web style navigation metaphor for a <code>TreeViewer</code>. Home, back, and "drill into"
 * functions are supported for the viewer,
 * <p>
 ...
 * <li>Create a container for your viewer with a toolbar or a popup menu. Add actions for "goBack", "goHome", and
 * "goInto" to either one by calling </code>addNavigationActions</code> with the popup menu or toolbar.</li>
 * </ol>
 * <p>
 * If the input for the underlying viewer is changed by something other than the adapter the <code>reset</code> method
 * should be called. This will clear the drill stack and update the navigation buttons to reflect the new state of the
 * underlying viewer.
 * </p>
 * </p>
 */
public class DrillDownAdapter implements ISelectionChangedListener
{
	 ...
	 * @return <code>true</code> if expansion is possible; otherwise return <code>false</code
	 */
	...
	/**
	 * Returns whether "go back" is possible for child tree. This is only possible if the client has performed one or
	...
	/**
	 * Returns whether "go home" is possible for child tree. This is only possible if the client has performed one or
	 * more drilling operations.
	 * 
	...
	/**
	 * Returns whether "go into" is possible for child tree. This is only possible if the current selection in the
	 * client has one item and it has children.
	 * 
	...
	private void createActions()
	{
		...
		// Home.
		homeAction = new Action(homeActionString)
		{
			public void run()
			{
				goHome();
		...
		homeAction.setToolTipText(homeActionString);
		ImageDescriptor id = WorkbenchImages.getImageDescriptor("IMG_ETOOL_HOME_NAV"); //$NON-NLS-1$
		if(id != null)
		{
			homeAction
		...
		backAction = new Action(backActionString)
		{
			public void run()
			{
				goBack();
		...
		backAction.setToolTipText(backActionString);
		...
		// Forward.
		forwardAction = new Action(forwardActionString)
		{
			public void run()
			{
				goInto();
		...
		forwardAction.setToolTipText(forwardActionString);
	...
	/**
	 * Expands the given items in the tree. The list of items passed should be derived by calling
	 * <code>getExpanded</code>.
	 * 
	 * @param items
	 *            is a list of items within the tree which should be expanded
	 */
	 ...
	 * 
	 * @return a list of expanded elements
	 */
	...
	/**
	 * Reverts the input for the tree back to the state when <code>goInto</code> was last called.
	 * <p>
	 * A frame is removed from the drill stack. Then that frame is used to reset the input and expansion state for the
	 * child tree.
	 * </p>
	 */
	public void goBack()
	 ...
	 * <p>
	 * All of the frames are removed from the drill stack. Then the oldest frame is used to reset the input and
	 * expansion state for the child tree.
	 * </p>
	 */
	public void goHome()
	{
	 ...
	 * <p>
	 * The current input and expansion state are saved in a frame and added to the drill stack. Then the input for the
	 * tree is changed to be the current selection. The expansion state for the tree is maintained during the operation.
	 * </p>
	 * <p>
	 * On return the client may revert back to the previous state by invoking <code>goBack</code> or
	 * <code>goHome</code>.
	 * </p>
	 */
	public void goInto()
	 ...
	 * <p>
	 * The current input and expansion state are saved in a frame and added to the drill stack. Then the input for the
	 * tree is changed to be <code>newInput</code>. The expansion state for the tree is maintained during the
	 * operation.
	 * </p>
	 * <p>
	 * On return the client may revert back to the previous state by invoking <code>goBack</code> or
	 ...
	 * <p>
	 * This method is typically called when the input for the underlying view is reset by something other than the
	 * adapter. On return the drill stack has been cleared and the navigation buttons reflect the new state of the
	 * underlying viewer.
	 * </p>
	 */
	public void reset()
