...
public class WebBrowserEditor extends EditorPart implements IBrowserViewerContainer {
	...
	public static final String WEB_BROWSER_EDITOR_ID = "com.aptana.ide.core.ui.browser.editor"; //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		try {
			webBrowser = new BrowserViewer(parent, style | SWT.MOZILLA);
		} catch (SWTError e) {
	...
	public static WebBrowserEditor openBlank() {
		return open((URL)null);
	}

	public static WebBrowserEditor open(URL url) {
		return open(url, UUID.randomUUID().toString());
	}

	public static WebBrowserEditor open(URL url, String browserId) {
		int style = BrowserViewer.LOCATION_BAR | BrowserViewer.BUTTON_BAR;
		WebBrowserEditorInput input = new WebBrowserEditorInput(url, style, browserId);
		if (url != null) {
			input.setName(url.getFile());
			input.setToolTipText(url.toExternalForm());
		}
		...
		try {
			...
			return (WebBrowserEditor) page.openEditor(input, WebBrowserEditor.WEB_BROWSER_EDITOR_ID);
		} catch (Exception e) {
			IdeLog.logImportant(CoreUIPlugin.getDefault(), "Error opening Web browser", e); //$NON-NLS-1$
		}
		return null;
	}
	
	public static void open(WebBrowserEditor webBrowserEditor, URL url) {
		String browserId = UUID.randomUUID().toString();
		int style = 0;
		if (webBrowserEditor.getEditorInput() instanceof WebBrowserEditorInput) {
			browserId = ((WebBrowserEditorInput) webBrowserEditor.getEditorInput()).getBrowserId();
		}
		WebBrowserEditorInput input = new WebBrowserEditorInput(url, style, browserId);
		if (url != null) {
			input.setName(url.getFile());
		...
		try {
			webBrowserEditor.init(webBrowserEditor.getEditorSite(), input);
		} catch (Exception e) {
			IdeLog.logImportant(CoreUIPlugin.getDefault(), "Error opening Web browser", e); //$NON-NLS-1$
		}
	}
