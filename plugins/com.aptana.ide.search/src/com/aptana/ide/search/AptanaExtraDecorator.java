...
final class AptanaExtraDecorator implements ILabelDecorator
{
	 ...
	 */
	AptanaExtraDecorator(AptanaFileSearchPage aptanaFileSearchPage)
	{
	...
	{
		if (this.aptanaFileSearchPage.turnOff)
		{
			return text;
		}
		if (element instanceof IResource)
		{
			if (!(element instanceof IFile))
			{
				...
				String result = MessageFormat.format(format, new Object[] { text, new Integer(matches) });
				return result;
			}
		}
		return text;
	...
	private int count(IResource rs)
	{
		...
		Object[] children = this.aptanaFileSearchPage.provider.getChildren(rs);
		int matches = 0;
		for (int a = 0; a < children.length; a++)
		{
			Object object = children[a];
			if (object instanceof IFile)
			{
				matches += this.aptanaFileSearchPage.getInput().getMatchCount(object);
			}
			else
			{
