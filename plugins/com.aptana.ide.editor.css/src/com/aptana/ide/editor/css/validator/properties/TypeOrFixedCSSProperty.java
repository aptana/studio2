...
/**
 * CSS property that accepts values of some fixed types set and also
 * accept predefined values list.
 *  
...
public abstract class TypeOrFixedCSSProperty extends PlugableCSSProperty
{
    ...
    /**
     * TypeOrFixedCSSProperty constructor.
     */
     ...
     */
    public TypeOrFixedCSSProperty(String propertyName, Class[] types,
            CSSValueTypeConverter[] converters, String[] values,
            ApplContext context, CssExpression expression, boolean check)
            throws InvalidParamException
    {
        super(propertyName, getPropertyTypes(types), converters, 
                new CSSValueValidator[]{new FixedSetIdentifierValidator(values)},
                context, expression, check);
    }
     ...
     * @param originalTypes - original types.
     * @return new types
     */
    private static Class[] getPropertyTypes(Class[] originalTypes)
    {
        ...
        {
            if (CssIdent.class.equals(originalTypes[i]))
            {
