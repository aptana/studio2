...
public class CSSPageNode extends CSSParseNode
{
	 ...
	 * @param startLexeme
	 */
	public CSSPageNode(Lexeme startLexeme)
	{
		super(CSSParseNodeTypes.PAGE, startLexeme);
	}
	...
	{
		writer.printWithIndent("@page"); //$NON-NLS-1$
		
		if (this.hasAttribute("name")) //$NON-NLS-1$
		{
			writer.print(":").print(this.getAttribute("name")); //$NON-NLS-1$ //$NON-NLS-2$
		}
		
		if (this.hasChildren())
		{
			if (this.getChildCount() == 1 && this.getChild(0).getChildCount() == 0)
			{
			...
			else
			{
				...
				for (IParseNode node : this.getChildren())
				{
