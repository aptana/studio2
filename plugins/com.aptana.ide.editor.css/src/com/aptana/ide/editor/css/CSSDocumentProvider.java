 ...
 */
public final class CSSDocumentProvider extends BaseDocumentProvider
{
	/**
	 * Bundle of all required informations to allow working copy management.
	 */
	protected static class CSSFileInfo extends FileInfo
	{
		/**
		 * sourceProvider
		 */
	...
	private static CSSDocumentProvider _unifiedDocumentProvider = null;
	 ...
	 */
	public static CSSDocumentProvider getInstance()
	{
		if (_unifiedDocumentProvider == null)
		{
			_unifiedDocumentProvider = new CSSDocumentProvider();
		}

		return _unifiedDocumentProvider;
	}

	private CSSDocumentProvider()
	{
	}
	...
	protected FileInfo createFileInfo(Object element) throws CoreException
	{
		...
		try
		{
			...
			if (!(info instanceof CSSFileInfo))
			{
				return null;
			}
			
			CSSFileInfo cuInfo = (CSSFileInfo)info;
			if(element instanceof IAdaptable)
			{
				// If this is check is not added, workspace files (those in projects) lose their annotations (like debugger markers)
		...
		{
			//TODO:
			IdeLog.logError(UnifiedEditorsPlugin.getDefault(), Messages.CSSDocumentProvider_CantCreateFileInfo, ex);
		}
		return null;
	}

	// /**
	// * Returns the language service for the given editor input
	// * @param element
	// * @param mimeType
	// * @return Returns the language service for the given editor input
	// */
	// public IFileLanguageService getFileLanguageService(IEditorInput element, String mimeType)
	// {
	// IFileContext fs = getFileService(element);
	// if(fs != null)
	// return fs.getLanguageService(mimeType);
	// return null;
	// }

	// /**
	// * Returns the file service for the given editor input
	// * @param element
	// * @return Returns the language service for the given editor input
	// */
	// public IFileContext getFileService(IEditorInput element)
	// {
	// FileInfo fileInfo= getFileInfo(element);
	// if (fileInfo instanceof UnifiedFileInfo) {
	// UnifiedFileInfo info= (UnifiedFileInfo) fileInfo;
	// return info.fileService;
	// }
	// return null;
	// }
	...
	{
		CSSFileInfo cuInfo = (CSSFileInfo) getFileInfo(element);
		...
		{
			//TODO:
			IdeLog.logError(UnifiedEditorsPlugin.getDefault(), Messages.CSSDocumentProvider_ErrorDisconnectingDocumentProvider, ex);
		}
	}
