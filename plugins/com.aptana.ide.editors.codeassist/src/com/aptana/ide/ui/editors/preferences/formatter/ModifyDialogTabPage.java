...
public abstract class ModifyDialogTabPage {
	...
	protected class ButtonPreference extends Preference {
		...
		/**
		 * @see com.aptana.ide.ui.editors.preferences.formatter.ModifyDialogTabPage.Preference#updateWidget()
		 */
		...
		/**
		 * @see com.aptana.ide.ui.editors.preferences.formatter.ModifyDialogTabPage.Preference#getControl()
		 */
	...
	public final class NumberPreference extends Preference {
		...
		private IStatus createErrorStatus() {
		    return new Status(IStatus.ERROR, Activator.PLUGIN_ID, 0, StringUtils.format(FormatterMessages.ModifyDialogTabPage_NumberPreference_error_invalid_value, new String [] {Integer.toString(fMinValue), Integer.toString(fMaxValue)}), null); 		    
		}
		...
		public void updateWidget() {
			...
			if (hasKey) {
			    ...
			    try {
			        ...
			        String message= StringUtils.format(FormatterMessages.ModifyDialogTabPage_NumberPreference_error_invalid_key, getKey()); 
			        Activator.getDefault().getLog().log(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, message, e));
			        s= ""; //$NON-NLS-1$
			    }
	...
	protected final static class DefaultFocusManager extends FocusAdapter {
		...
		private final static String PREF_LAST_FOCUS_INDEX= Activator.PLUGIN_ID + "formatter_page.modify_dialog_tab_page.last_focus_index"; //$NON-NLS-1$ 
	...
	/**
	 * Convenience method to create a RadioPreference. The widget is registered as 
	 * a potential focus holder, and the default updater is added.
	 * 
	 * @param composite
	 * @param numColumns
