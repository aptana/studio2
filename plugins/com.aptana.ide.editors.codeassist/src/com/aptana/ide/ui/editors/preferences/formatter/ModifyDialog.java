...
import com.aptana.ide.ui.editors.preferences.formatter.ModifyDialogTabPage.IModificationListener;
import com.aptana.ide.ui.editors.preferences.formatter.ProfileManager.CustomProfile;
...
public abstract class ModifyDialog extends StatusDialog implements IModificationListener {
	...
	public ModifyDialog(Shell parentShell, Profile profile, ProfileManager profileManager, ProfileStore profileStore, boolean newProfile, String dialogPreferencesKey, String lastSavePathKey) {
		...
		fKeyPreferredWidth= Activator.PLUGIN_ID + dialogPreferencesKey + DS_KEY_PREFERRED_WIDTH;
		fKeyPreferredHight= Activator.PLUGIN_ID + dialogPreferencesKey + DS_KEY_PREFERRED_HEIGHT;
		fKeyPreferredX= Activator.PLUGIN_ID + dialogPreferencesKey + DS_KEY_PREFERRED_X;
		fKeyPreferredY= Activator.PLUGIN_ID + dialogPreferencesKey + DS_KEY_PREFERRED_Y;
		fKeyLastFocus= Activator.PLUGIN_ID + dialogPreferencesKey + DS_KEY_LAST_FOCUS;
		...
		fProfile= profile;
		setTitle(StringUtils.format(FormatterMessages.ModifyDialog_dialog_title, profile.getName()));
    ...
    protected void buttonPressed(int buttonId) {
		if (buttonId == APPLAY_BUTTON_ID) {
			applyPressed();
			setTitle(StringUtils.format(FormatterMessages.ModifyDialog_dialog_title, fProfile.getName()));
	...
	private void saveButtonPressed() {
		Profile selected= new CustomProfile(fProfileNameField.getText(), new HashMap(fWorkingValues), fProfile.getVersion(), ""); //$NON-NLS-1$
		...
		if (file.exists() && !MessageDialog.openQuestion(getShell(), FormatterMessages.CodingStyleConfigurationBlock_save_profile_overwrite_title, StringUtils.format(FormatterMessages.CodingStyleConfigurationBlock_save_profile_overwrite_message, path))) { 
			return;
		}
...
//			final String message= FormatterMessages.CodingStyleConfigurationBlock_save_profile_error_message;
	...
	/**
	 * @see com.aptana.ide.ui.editors.preferences.formatter.ModifyDialogTabPage.IModificationListener#valuesModified()
	 */
    ...
    private void doValidate() {
		...
		if (!name.equals(fProfile.getName()) && fProfileManager.containsName(name)) {
			updateStatus(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, FormatterMessages.ModifyDialog_Duplicate_Status, null));
			return;
		}
		if (fProfile.isBuiltInProfile() || fProfile.isSharedProfile()) {
			updateStatus(new Status(IStatus.INFO, Activator.PLUGIN_ID, IStatus.OK, FormatterMessages.ModifyDialog_NewCreated_Status, null));
			return;
		}
    ...
    private IStatus validateProfileName() {
	    ...
	    if (fProfile.isBuiltInProfile()) {
			if (fProfile.getName().equals(name)) { 
				return new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, FormatterMessages.ModifyDialog_BuiltIn_Status, null);
			}	
    	}
    	
    	if (fProfile.isSharedProfile()) {
			if (fProfile.getName().equals(name)) { 
				return new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, FormatterMessages.ModifyDialog_Shared_Status, null);
			}
    	}
		
		if (name.length() == 0) {
			return new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, FormatterMessages.ModifyDialog_EmptyName_Status, null);
		}
