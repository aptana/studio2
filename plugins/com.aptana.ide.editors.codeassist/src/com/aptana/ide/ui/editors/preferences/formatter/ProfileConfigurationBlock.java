...
import com.aptana.ide.internal.ui.dialogs.SWTUtil;
			...
			try {
    			...
    			case ProfileManager.SETTINGS_CHANGED_EVENT:
    				try {
    					...
    					Activator.getDefault().getLog().log(x.getStatus());
    				}
    				break;
	...
	class ButtonController implements Observer, SelectionListener {
		...
		private void deleteButtonPressed() {
			if (MessageDialog.openQuestion(
					...
					StringUtils.format(FormatterMessages.CodingStyleConfigurationBlock_delete_confirmation_question, fProfileManager.getSelected().getName()))) { 
		...
		private void loadButtonPressed() {
			...
			final String lastPath= Activator.getDefault().getDialogSettings().get(fLastSaveLoadPathKey + ".loadpath"); //$NON-NLS-1$
			if (lastPath != null) {
			...
			if (path == null) 
				return;
			Activator.getDefault().getDialogSettings().put(fLastSaveLoadPathKey + ".loadpath", dialog.getFilterPath()); //$NON-NLS-1$
...
//				final String message= StringUtils.format(FormatterMessages.ProfileConfigurationBlock_load_profile_wrong_profile_message, new String[] {fProfileVersioner.getProfileKind(), profile.getKind()});
...
//			}
	 ...
	 * @param pluginId
	 */
	public ProfileConfigurationBlock(IProject project, final PreferencesAccess access, String lastSaveLoadPathKey, String pluginId) {
	 ...
	 */
	protected abstract ProfileManager createProfileManager(List profiles, IScopeContext context, PreferencesAccess access);
