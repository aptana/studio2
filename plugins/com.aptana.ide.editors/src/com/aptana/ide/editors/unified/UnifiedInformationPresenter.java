 ...
 * @author Paul Colton
 */
public class UnifiedInformationPresenter extends AbstractInformationControlManager implements IInformationPresenter,
		IInformationPresenterExtension, IWidgetTokenKeeper, IWidgetTokenKeeperExtension
{

	/**
	 * Priority of the info controls managed by this information presenter. Default value:
	 * <code>5</code>.
	 * 
	 * @since 3.0
	 */
	/*
	 * 5 as value has been chosen in order to beat the hovers of
	 * {@link org.eclipse.jface.text.TextViewerHoverManager}
	 */
	...
	/**
	 * Creates a new information presenter that uses the given information control creator. The
	 * presenter is not installed on any text viewer yet. By default, an information control closer
	 * is set that closes the information control in the event of key strokes, resizing, moves,
	 * focus changes, mouse clicks, and disposal - all of those applied to the information control's
	 * parent control. Also, the setup ensures that the information control when made visible will
	 * request the focus. By default, the default document partitioning
	 * {@link IDocumentExtension3#DEFAULT_PARTITIONING} is used.
	 * 
	 * @param creator
	 *            the information control creator to be used
	 */
	public UnifiedInformationPresenter(IInformationControlCreator creator)
	{
		super(creator);
	...
	/**
	 * Registers a given information provider for a particular content type. If there is already a
	 * provider registered for this type, the new provider is registered instead of the old one.
	 * 
	 * @param provider
	 *            the information provider to register, or <code>null</code> to remove an existing
	 *            one
	 * @param contentType
	...
	/**
	 * Translated the given range in the viewer's document into the corresponding range of the
	 * viewer's widget.
	 * 
	 * @param region
	...
	protected void showInformationControl(Rectangle subjectArea)
	{
		 ...
		 * extension= (IWidgetTokenOwnerExtension) fTextViewer; if
		 * (extension.requestWidgetToken(this, WIDGET_PRIORITY))
		 * super.showInformationControl(subjectArea); } else if (fTextViewer instanceof
		 * IWidgetTokenOwner) { IWidgetTokenOwner owner= (IWidgetTokenOwner) fTextViewer; if
		 * (owner.requestWidgetToken(this)) super.showInformationControl(subjectArea); }
		 */
	}
