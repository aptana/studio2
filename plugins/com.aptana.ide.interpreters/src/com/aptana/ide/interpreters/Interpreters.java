...
public class Interpreters
{
	 ...
	 * @param language - language to get interpreters for.
	 ...
	 * @param language - language to get default interpreter for.
				...
				for (int j = 0; j < elements.length; j++)
				{
					...
					if (elementName.equals(PROVIDER_ELEMENT_NAME))
					{
						...
						if (priorityString != null)
						{
							...
							{
								IdeLog.logError(IDECorePlugin.getDefault(), 
										Messages.getString("Interpreters.ERR_WrongPriorityFormatPrefix") + elementName  //$NON-NLS-1$
										+ Messages.getString("Interpreters.ERR_WrongPriorityFormatSuffix")); //$NON-NLS-1$
							}
						}
						try
						{
							IInterpreterProvider provider = 
								(IInterpreterProvider) element.createExecutableExtension(CLASS_ATTRIBUTE_NAME);
						...
						{
							IdeLog.logError(IDECorePlugin.getDefault(), Messages.getString("Interpreters.ERR_UnableCreatingProvider"), e); //$NON-NLS-1$
						}
					}
				}
			}
