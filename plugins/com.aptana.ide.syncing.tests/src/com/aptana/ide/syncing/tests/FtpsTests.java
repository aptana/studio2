 ...
 */
public class FtpsTests extends TestCase
{
	// private static final String server = "ftp12.asphost4free.com"; //$NON-NLS-1$
	// private static final String user = "aptana"; //$NON-NLS-1$
	// private static final String pass = System.getProperty("ftp.password"); //$NON-NLS-1$
	// private static final String home = "/aptana"; //$NON-NLS-1$
	private static final String nickname = "Aptana FTPS Test Site"; //$NON-NLS-1$
	...
	{
		FtpsProtocolManager protocolManager = new FtpsProtocolManager();

		this._manager = (FtpsVirtualFileManager) protocolManager.createFileManager();
		...
		this._manager.setExplicitMode(true);
		this._manager.setPassiveMode(true);
		this._manager.setSecurityType(FtpsVirtualFileManager.SSL);
	}
	...
	public void testGetFiles() throws IOException, ConnectionException, InterruptedException
	{
		...
		// Create some files
		IVirtualFile file1 = this._manager.createVirtualFile(path + this._manager.getFileSeparator()
				+ FileUtils.getRandomFileName("test1", ".txt")); //$NON-NLS-1$ //$NON-NLS-2$
		IVirtualFile file2 = this._manager.createVirtualFile(path + this._manager.getFileSeparator()
				+ FileUtils.getRandomFileName("test2", ".txt")); //$NON-NLS-1$ //$NON-NLS-2$
		createFile(file1);
		createFile(file2);
