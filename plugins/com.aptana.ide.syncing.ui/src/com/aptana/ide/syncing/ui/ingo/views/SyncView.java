...
import com.aptana.ide.core.io.ingo.IVirtualFileManagerDialog;
...
public class SyncView extends ViewPart implements ISyncManagerChangeListener
{
	...
	private TableViewer createConnectionTable(Composite parent)
	{
		...
		imageCol.pack();
		locationCol.pack();
		PreferenceUtils.persist(SyncingUIPlugin.getDefault().getPreferenceStore(), serverTable, "syncView"); //$NON-NLS-1$
		return view;
	}
	...
	private void startSynchronize()
	{
		...
		if (element instanceof VirtualFileManagerSyncPair)
		{
			...
			else
			{
				...
				try {
					dialog = new SmartSyncDialog(CoreUIUtils.getActiveShell(), firstElement
							.getSourceFileManager(), firstElement
							.getDestinationFileManager(), firstElement
							.getSourceFileManager().getRoot(), firstElement.getDestinationFileManager().getRoot(),
		...
		else if (element instanceof IVirtualFileManager)
		{
			...
			IVirtualFileManagerDialog nld = conf.getProtocolManager().createPropertyDialog(SWT.DIALOG_TRIM | SWT.RESIZE | SWT.APPLICATION_MODAL);
						...
						if (conf.isValid())
						{
							try {
								name = StringUtils.format(Messages.SyncView_SyncConnectionLocationLabel, new String[] {
										conf.getSourceFileManager().getRoot().getName(),
