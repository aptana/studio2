			...
			IConnectionPoint destinationManager, String comment)
	{
		...
		for (int i = 0; i < listens.length; i++)
		{
			if (listens[i] instanceof ISmartSyncListener)
			{
				...
				{
					IdeLog.logError(AptanaCorePlugin.getDefault(), Messages.SmartSyncEventManager_ERR_ExceptionNotifyingSmartSyncListener, e);
				}
				catch (Error e)
				{
					IdeLog.logError(AptanaCorePlugin.getDefault(), Messages.SmartSyncEventManager_ERR_ErrorNotifyingSmartSyncListener, e);
				}
			}
		}
	}
