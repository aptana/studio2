...
/**
 * Factory that creates log watchers.
 * 
...
public class LogWatcherFactory
{
	...
	/**
	 * Creates watcher for resource.
	 * 
	 * @param resource
	 *            - resource to create watcher for.
	 * @param config
	...
	public static ILogWatcher createWatcher(ILogResource resource, LogWatcherConfiguration config)
	{
		...
		else if (resource instanceof WebLogResource)
		{
			try
			{
				String protocol = resource.getURI().toURL().getProtocol();
				if (protocol.equals("sftp")) //$NON-NLS-1$
	...
	/**
	 * LogWatcherFactory private constructor.
	 */
