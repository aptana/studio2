var AptanaIdManagerModule = Class.create(
{
	...
	showLoginWindow: function showLoginWindow(customMessage)
	{
        ...
        {
			Portal.API.dialogs.alert('You cannot log in while offline.  Please try again when you have internet connectivity', 'Offline Mode');
	...
	showCreateWindow: function showCreateWindow()
	{
		if (!Portal.Modules.OnlineStatus.isOnline) 
        {
			Portal.API.dialogs.alert('You cannot create an Aptana ID while offline.  Please try again when you have internet connectivity', 'Offline Mode');
	...
	logOutUser: function logOutUser(msg)
	{
		if(!msg)
		{
			...
			{
				if(Portal.Modules.AptanaIdManager.isLoggedIn == true && this.logoutRetries < 3)
				{
	...
	createNewUser: function(msg) 
	{
		if(!msg)
		{
            ...
            EventManager.publish('/portal/user/create', 
            {
				...
				last_name: lastname,
                email: email,
				captcha_token: $F('captcha_token'),
				captcha_response: $F('captcha_response')
            });
