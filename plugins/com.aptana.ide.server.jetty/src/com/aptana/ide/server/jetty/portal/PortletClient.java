 ...
 */
public class PortletClient extends CometResponderClient
{
	...
	{
		PortalService[] portlets = PortalServiceLoader.getLoader().getPortlets();
		String[] ids = new String[portlets.length];
		for (int i = 0; i < portlets.length; i++)
		{
			ids[i] = "/portlets/" + portlets[i].getId(); //$NON-NLS-1$
		}
		return ids;
	}
	...
	{
		return new String[] { "/portal/portlets" }; //$NON-NLS-1$
	}
