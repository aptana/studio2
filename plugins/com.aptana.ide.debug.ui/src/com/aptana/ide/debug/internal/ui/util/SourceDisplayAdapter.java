...
public class SourceDisplayAdapter implements InvocationHandler {
	...
	private static final String DEFAULT_SOURCE_LOCATOR_ID = "com.aptana.ide.debug.core.sourceLookupDirector"; //$NON-NLS-1$
	private static final String DEFAULT_SOURCE_PATH_COMPUTER_ID = "com.aptana.ide.debug.core.sourcePathComputer"; //$NON-NLS-1$
	...
	static {
		try {
			...
			try {
				clazz = Class.forName(Messages.getString("SourceDisplayAdapter.3")); //$NON-NLS-1$
			...
			if (!monitor.isCanceled() && result != null && page != null) {
				if (result.getEditorInput() == null) {
					MessageDialog.openError(page.getWorkbenchWindow().getShell(),
							...
							StringUtils.format(Messages.getString("SourceDisplayAdapter.Source_Not_Located"), //$NON-NLS-1$
