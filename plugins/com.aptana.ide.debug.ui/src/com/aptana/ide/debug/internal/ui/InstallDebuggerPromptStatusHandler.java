	...
	public Object handleStatus(IStatus status, Object source) throws CoreException
	{
		...
		if ("install".equals(source)) { //$NON-NLS-1$
			...
			MessageDialog md = new MessageDialog(
					...
					new String[] {
						StringUtils.ellipsify(Messages.InstallDebuggerPromptStatusHandler_Download),
			...
			MessageDialog.openInformation(shell, title, StringUtils.format(Messages.InstallDebuggerPromptStatusHandler_BrowserIsRunning, new String[] { ((String)source).substring(5)}));
			return null;
		}
		else if ( source instanceof String && ((String)source).startsWith("installed_") ) { //$NON-NLS-1$
			MessageDialog.openInformation(shell, title, StringUtils.format(Messages.InstallDebuggerPromptStatusHandler_ExtensionInstalled, new String[] { ((String)source).substring(10)}));
			return null;
		}
		...
		String message = StringUtils.format(Messages.InstallDebuggerPromptStatusHandler_ExtensionNotInstalled, new String[] { (String)source });
