 ...
 */
public class UserRequestBuilder implements IServiceRequestBuilder
{
	...
	public IServiceRequest generateRequest(ILocationObject object, String type)
	{
		...
		if (object instanceof User)
		{
			...
			if (IServiceRequest.COMMIT.equals(type))
			{
				request = new SimpleServiceRequest("application/xml", user.toXML(), type, null); //$NON-NLS-1$
			}
			else
			{
				if (object.hasLocation())
				{
					request = new SimpleServiceRequest("application/xml", "", type, AuthUtils.getAuthorizationHeader( //$NON-NLS-1$ //$NON-NLS-2$
							user.getUsername(), user.getPassword()));
				}
				else if (object.getDefaultLocation() != null)
				{
					request = new SimpleServiceRequest("application/xml", user.toXML(), IServiceRequest.COMMIT, null); //$NON-NLS-1$
				}
			}
		}
		return request;
