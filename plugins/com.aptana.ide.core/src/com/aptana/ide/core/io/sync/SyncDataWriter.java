...
/**
 * The SyncDataWriter is a class that imitate {@link WorkspaceSaveParticipant},
 * however, it maintains the version of the saved file and make sure a rollback
...
public class SyncDataWriter {
    ...
    private static final SaveManager SAVE_MANAGER = ((Workspace) ResourcesPlugin.getWorkspace())
            .getSaveManager();

    /**
     * Tells this writer that the save operation is now complete and it is free
     * to go about its normal business. Exceptions are not expected to be thrown
     * at this point, so they should be handled internally.
     */
    public void doneSaving() {
        AptanaCorePlugin plugin = AptanaCorePlugin.getDefault();
        // Since there is a chance that the save number will change during the
        // workspace life-cycle, we do not
        // cache this number.
        int saveNumber = SAVE_MANAGER.getSaveNumber(AptanaCorePlugin.ID);
