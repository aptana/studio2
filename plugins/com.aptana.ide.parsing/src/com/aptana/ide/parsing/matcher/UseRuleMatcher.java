...
public class UseRuleMatcher extends AbstractLexemeMatcher
{
	...
	public int match(Lexeme[] lexemes, int offset, int eofOffset)
	{
		...
		ParserMatcher parser = this.getOwningParser();
		
		if (parser != null)
		{
			RuleMatcher rule = parser.getRuleByName(this.getName());
			
			if (rule != null)
			{
				result = rule.match(lexemes, offset, eofOffset);
			}
			
			if (result != -1)
			{
				this.result = rule.getParseResults();
			}
		}
