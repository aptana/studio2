 ...
 */
public class OptionalMatcher extends AbstractTextMatcher
{
	/**
	 * OneOrMore
	 */
	...
	public int match(char[] source, int offset, int eofset)
	{
		...
		{
			ITextMatcher child = (ITextMatcher) this.getChild(0);
			int temp = child.match(source, offset, eofset);
	...
	protected void validateLocal()
	{
		...
		if (count == 0)
		{
			this.getDocument().sendError(Messages.OptionalMatcher_No_Children, this);
		}
		else if (count > 1)
		{
			this.getDocument().sendInfo(Messages.OptionalMatcher_Wrapping_Children, this);
		}
	}
}
