...
public class OneOrMoreMatcher extends AbstractTextMatcher
{
	...
	public int match(char[] source, int offset, int eofOffset)
	{
		...
		if (this.getChildCount() > 0)
		{
			...
			result = child.match(source, offset, eofOffset);
			
			while (result != -1)
			{
				int temp = child.match(source, result, eofOffset);
	...
	protected void validateLocal()
	{
		...
		if (count == 0)
		{
			this.getDocument().sendError(Messages.OneOrMoreMatcher_No_Children, this);
		}
		else if (count > 1)
		{
			this.getDocument().sendInfo(Messages.OneOrMoreMatcher_Wrapping_Children, this);
		}
	}
}
