...
public class LookaheadMatcher extends AbstractTextMatcher
{
	...
	public int match(char[] source, int offset, int eofset)
	{
		...
		{
			ITextMatcher matcher = (ITextMatcher) this.getChild(0);
			boolean match = (matcher.match(source, offset, eofset) != -1);
			
			if (match != this._negate)
			{
				result = offset;
			}
		}
	...
	protected void validateLocal()
	{
		...
		if (count == 0)
		{
			this.getDocument().sendError(Messages.LookaheadMatcher_No_Children, this);
		}
		else if (count > 1)
		{
			this.getDocument().sendInfo(Messages.LookaheadMatcher_Wrapping_Children, this);
		}
	}
}
