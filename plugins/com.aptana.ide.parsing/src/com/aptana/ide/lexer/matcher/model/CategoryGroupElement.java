...
public class CategoryGroupElement extends MatcherElement
{
	...
	{
		this.addChildType(IMatcherElement.class);
	}
	...
	public void createTokens(ITokenList tokenList)
	{
		...
		while (this.getChildCount() > 0)
		{
			...
			{
				IMatcherElement matcher = (IMatcherElement) child;

				if (tokenList instanceof MatcherTokenList && matcher instanceof ITextMatcher)
				{
					String group = matcher.getGroup();

					((MatcherTokenList) tokenList).addMatcherToGroup((ITextMatcher) matcher, group);
				}
			}
	...
	protected void validateLocal()
	{
		...
		{
			this.getDocument().sendError(Messages.CategoryGroupElement_Missing_Category, this);
		}
	}
}
