 ...
 */
public final class XMLFileServiceFactory extends BaseFileServiceFactory
{
	private static XMLFileServiceFactory instance;

	/**
	 * XMLFileServiceFactory
	 */
	...
	protected IParser createScanner()
	{
		...
		{
			result = LanguageRegistry.getScanner(XMLMimeType.MimeType);
		}
	...
	public FileService createFileService(IFileSourceProvider sourceProvider, boolean parse)
	{
		...
		FileService fileService = new FileService(parser, parseState, sourceProvider, XMLMimeType.MimeType);

		fileService.setScanner(scanner);
		fileService.setErrorManager(new XMLErrorManager(fileService));

		IParentOffsetMapper mapper = new ParentOffsetMapper(fileService);

		XMLFileLanguageService languageService = new XMLFileLanguageService(fileService, parseState, parser, mapper);
		fileService.addLanguageService(XMLMimeType.MimeType, languageService);
	...
	protected IParser createParser()
	{
		...
		if (parser == null)
		{
			IdeLog.logError(
				...
				StringUtils.format(
					Messages.XMLFileServiceFactory_ERR_CreateParser,
