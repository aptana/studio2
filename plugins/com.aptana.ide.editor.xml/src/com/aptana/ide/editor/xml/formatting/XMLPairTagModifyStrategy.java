 ...
 */
public class XMLPairTagModifyStrategy implements IAutoEditStrategy,
        IPreferenceClient
{
            ...
            DocumentCommand command)
    {
        ...
        //modifies pair tag
        modifyPairTag(document, command, xmlParseState, store);
    }
            ...
            IParseState parseState, IPreferenceStore store)
    {
        ...
        {
            XMLPairFinder finder = new XMLPairFinder();
            PairMatch match = 
            ...
            LexemeList lexemes, Lexeme cursorLexeme, Lexeme pairLexeme)
    {
                ...
                && cursorEndingLexeme.getEndingOffset() == command.offset + command.length)
        {
            ...
            {
                //should not happen
                IdeLog.logError(XMLPlugin.getDefault(), 
                        "Unexpected exception while checking for complete tag removal", e); //$NON-NLS-1$
            }
        }
    ...
    private Lexeme getTagEndingLexeme(Lexeme startLexeme, LexemeList lexemes)
    {
        ...
        if ((startLexeme.typeIndex != XMLTokenTypes.START_TAG &&
                startLexeme.typeIndex != XMLTokenTypes.END_TAG) || startIndex < 0)
        {
            throw new IllegalArgumentException("Wrong start lexeme"); //$NON-NLS-1$
        }
        
        Lexeme closing = XMLUtils.getFirstLexemeBreaking(lexemes, startIndex + 1, 
                new int[]{XMLTokenTypes.GREATER_THAN},
                new int[]{XMLTokenTypes.START_TAG,
                    XMLTokenTypes.END_TAG,
                    XMLTokenTypes.START_TAG,}
        );
        return closing;
    }
    ...
    {
        if (tagLexeme.typeIndex == XMLTokenTypes.START_TAG)
        {
            return tagLexeme.getStartingOffset() + 1;
        }
        else if (tagLexeme.typeIndex == XMLTokenTypes.END_TAG)
        {
    ...
    private Lexeme getTagLexeme(IParseState state, int offset)
    {
        ...
        if (currentLexeme != null && (currentLexeme.typeIndex == XMLTokenTypes.START_TAG 
        ...
        if ((previousLexeme.typeIndex == XMLTokenTypes.START_TAG 
                || previousLexeme.typeIndex == XMLTokenTypes.END_TAG)
                && currentLexeme.typeIndex == XMLTokenTypes.GREATER_THAN)
        {
