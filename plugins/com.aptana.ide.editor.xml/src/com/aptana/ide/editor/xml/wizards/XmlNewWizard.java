 ...
 */
package com.aptana.ide.editor.xml.wizards;
...
/**
 * Wizard creates a new empty XML file.
 */
public class XmlNewWizard extends SimpleNewFileWizard
{
	/**
	...
	{
		super();
		setWindowTitle(Messages.XmlNewWizard_Title);
	}
	...
	protected SimpleNewWizardPage createNewFilePage(ISelection selection)
	{
		...
		page.setRequiredFileExtensions(new String[] { "xml" }); //$NON-NLS-1$
		page.setTitle(Messages.XmlNewWizard_Title2);
		page.setDescription(Messages.XmlNewWizard_Desc);

		IPreferenceStore store = XMLPlugin.getDefault().getPreferenceStore();
		String fileName = store.getString(IPreferenceConstants.XMLEDITOR_INITIAL_FILE_NAME);
	...
	protected String getInitialFileContents()
	{
		...
		IPreferenceStore store = XMLPlugin.getDefault().getPreferenceStore();
		String contents = store.getString(IPreferenceConstants.XMLEDITOR_INITIAL_CONTENTS);
		pw.print(contents);
		pw.close();
