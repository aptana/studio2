 ...
 */
public class LexerValidator extends ValidatorBase implements IErrorHandler
{
			...
			boolean collectErrors, boolean collectWarnings, boolean collectInfos)
	{
		...
		lexerParser.setErrorHandler(this);

		// conver source into a stream
		ByteArrayInputStream in = new ByteArrayInputStream(source.getBytes());
		
		// parse source and get result
		DocumentNode result = lexerParser.loadXML(in);
	...
	public void handleError(int line, int column, String message)
	{
		...
		{
			this._reporter.error(message, this._path, line, "", column); //$NON-NLS-1$
		}
	}
	...
	public void handleInfo(int line, int column, String message)
	{
		...
		{
			this._reporter.info(message, this._path, line, "", column); //$NON-NLS-1$
		}
	}
	...
	public void handleWarning(int line, int column, String message)
	{
		...
		{
			this._reporter.warning(message, this._path, line, "", column); //$NON-NLS-1$
		}
	}
}
