...
import com.aptana.ide.core.ui.views.fileexplorer.FileLabelProvider;
...
public class SmartSyncLabelProvider implements ITableLabelProvider, ITableColorProvider
{
	...
	public Image getColumnImage(Object element, int columnIndex)
	{
		if (columnIndex == 0)
		{
			if (element instanceof ISyncResource)
			{
				...
				{
					return SyncingPlugin.getImage("icons/no.png"); //$NON-NLS-1$
				}
				if (state == ISyncResource.SYNCED)
				{
					return SyncingPlugin.getImage("icons/ok.png"); //$NON-NLS-1$
				}
				return SyncingPlugin.getImage("icons/empty.png"); //$NON-NLS-1$
			}
		...
		else if (columnIndex == 1)
		{
			if (element instanceof ISyncResource)
			{
				...
				if (resource.isSkipped())
				{
					return SyncingPlugin.getImage("icons/checked.gif"); //$NON-NLS-1$
				}
				return SyncingPlugin.getImage("icons/unchecked.gif"); //$NON-NLS-1$
			}
		}
		return null;
	}
	...
	private static String getModificationDate(IVirtualFile file)
	{
		if (file == null)
		{
			return ""; //$NON-NLS-1$
		}
		Date d = new Date(file.getModificationMillis());
