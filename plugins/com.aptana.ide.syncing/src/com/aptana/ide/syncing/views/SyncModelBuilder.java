...
public final class SyncModelBuilder
{
	...
	public static SyncFolder buildSyncFolder(VirtualFileSyncPair[] pairs)
	{
		...
		List<IVirtualFile> ignoredFiles = new ArrayList<IVirtualFile>();
		for (VirtualFileSyncPair pair : pairs)
		{
			...
			if (virtualFile != null)
			{
				IPath realPath = new Path(virtualFile.getRelativePath());
		...
		String segment;
		boolean found;
		for (IVirtualFile file : ignoredFiles)
		{
			realPath = new Path(file.getRelativePath());
			parent = root;
			found = true;
			for (int i = 0; i < realPath.segmentCount(); i++)
			{
