...
public class SmartSyncViewer
{
	...
	private TreeViewer createContents(final Composite parent)
	{
				...
				if (!viewer.getSelection().isEmpty() && viewer.getSelection() instanceof IStructuredSelection)
				{
					...
					if (selection instanceof SyncFile)
					{
								...
								|| file.getSyncState() == SyncState.ServerItemIsNewer)
						{
							...
							File local = new File(pair.getSourceFile().getAbsolutePath());
							FileCompareEditorInput input = new FileCompareEditorInput(new CompareConfiguration())
							{

								protected void prepareFiles()
								{
									IVirtualFile file = pair.getDestinationFile();
									File temp = new File(FileUtils.systemTempDir + "/" //$NON-NLS-1$
											+ FileUtils.getRandomFileName("compare", ".txt")); //$NON-NLS-1$ //$NON-NLS-2$
									try
									{
										pair.getDestinationFile().getFileManager().putToLocalFile(file, temp);
									}
		...
		fShowDiffs.setImage(SyncingPlugin.getImage("icons/compare_view.gif")); //$NON-NLS-1$
