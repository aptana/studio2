 ...
 */
public class JSStringNode extends JSParseNode
{
	...
	{
		super(JSParseNodeTypes.STRING, startLexeme);
		...
		{
			this._quoteType = QuoteType.SINGLE_QUOTE;
		}

		this._unclosed = (startLexeme.getCategoryIndex() == TokenCategories.ERROR);
		
		if (this._unclosed)
		{
			this.setAttribute("text", text.substring(1, text.length())); //$NON-NLS-1$
		}
		else
		{
			this.setAttribute("text", text.substring(1, text.length() - 1)); //$NON-NLS-1$
		}
	}
	...
	{
		if (this._quoteType == QuoteType.DOUBLE_QUOTE)
		{
			writer.print("\"" + this.getAttribute("text")); //$NON-NLS-1$ //$NON-NLS-2$
			
			if (this._unclosed == false)
			{
