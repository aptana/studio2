 ...
 */
public final class JSCommentFileServiceFactory extends BaseFileServiceFactory
{
	private static JSCommentFileServiceFactory instance;

	/**
	 * JSCommentFileServiceFactory
	 */
	 ...
	 * getInstance
	 * 
	 * @return JSCommentFileServiceFactory
	 */
	...
	public FileService createFileService(IFileSourceProvider sourceProvider, boolean parse)
	{
		...
		FileService fileService = new FileService(parser, parseState, sourceProvider, JSCommentMimeType.MimeType);
		fileService.setScanner(scanner);
		// fileService.setErrorManager(new JSCommentErrorManager(fileService));
		
		IParentOffsetMapper mapper = new ParentOffsetMapper(fileService);

		JSCommentFileLanguageService languageService = new JSCommentFileLanguageService(fileService, parseState,
				parser, mapper);
		fileService.addLanguageService(JSCommentMimeType.MimeType, languageService);
	...
	{
		IParser parser = LanguageRegistry.getParser(JSCommentMimeType.MimeType);
	...
	protected IParser createScanner()
	{
		...
		{
			result = LanguageRegistry.getScanner(JSCommentMimeType.MimeType);
		}
