 ...
 */
package com.aptana.ide.ui.ftp.navigator.actions;
...
public class DoubleClickAction extends BaseDoubleClickAction {
    ...
    public void run() {
        if (selectionHasChildren()) {
            ...
            Dialog dialog = new FTPPropertyDialogProvider()
                    .createPropertyDialog(new IShellProvider() {
            ...
            if (dialog instanceof IPropertyDialog) {
                ((IPropertyDialog) dialog).setPropertySource(CoreIOPlugin
                        .getConnectionPointManager().getType(FTPConnectionPoint.TYPE));
            }
            dialog.open();
        }
    }
