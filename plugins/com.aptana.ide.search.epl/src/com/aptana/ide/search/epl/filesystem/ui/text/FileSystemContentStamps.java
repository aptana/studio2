...
/**
 * Modified to take java.io.File as the content control.
 */
public class FileSystemContentStamps {
    ...
    private static class ContentStampImpl extends ContentStamp {
        ...
        private static ContentStamp createDocumentStamp(long value,
                long fileValue) {
    ...
    public static ContentStamp get(File file, IDocument document) {
        if (document instanceof IDocumentExtension4) {
            long stamp = ((IDocumentExtension4) document)
                    .getModificationStamp();
            if (stamp == IDocumentExtension4.UNKNOWN_MODIFICATION_STAMP)
                return NULL_CONTENT_STAMP;
            return ContentStampImpl.createDocumentStamp(stamp, file
                    .lastModified());
        }
    ...
    public static boolean set(IDocument document, ContentStamp s)
        ...
        if (document instanceof IDocumentExtension4 && stamp.isDocumentStamp()) {
            try {
                ((IDocumentExtension4) document).replace(0, 0,
                        "", stamp.getValue()); //$NON-NLS-1$
                return true;
