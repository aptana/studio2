...
public class JSDebugThread extends JSDebugElement implements IThread
{
	...
	{
		return StringUtils.format(Messages.JSDebugThread_ThreadMain_0, (runningState == STATE_SUSPENDING ? StringUtils
	...
	private synchronized void getStackFrames0() throws DebugException
	{
		...
		if (args != null)
		{
			...
			for (int i = args.length - 1; i >= 1; --i)
			{
				...
				{
					function = StringUtils.format("[{0}]", //$NON-NLS-1$
							i == args.length - 1 ? Messages.JSDebugTarget_TopLevelScript : Messages.JSDebugTarget_EvalScript); 
				}
				else
				{
					function += StringUtils.format("({0})", arguments); //$NON-NLS-1$
				}
	...
	protected void stepToFrame(IStackFrame frame) throws DebugException
	{
		...
		runningState = STATE_STEPPPING;
		fireChangeEvent(DebugEvent.STATE);
		((JSDebugTarget) getDebugTarget()).getConnection().sendCommand(StringUtils.format(STEP_TO_FRAME_0, targetFrameId));
	}
}
