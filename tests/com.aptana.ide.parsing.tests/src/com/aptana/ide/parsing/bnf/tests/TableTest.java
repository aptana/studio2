...
public final class TableTest
{
	...
	{
		GrammarNode grammar = SampleGrammar.getGrammar();
		// GrammarNode grammar = BNFGrammar.getGrammar();
		Parser generator = new Parser();

		generator.generateTable(grammar);

		State[] states = generator.getStates();
		IGrammarNode[] symbols = grammar.getSymbols();
		...
		for (int i = 0; i < states.length; i++)
		{
			...
			{
				IGrammarNode symbol = symbols[j];
				Action action = state.getAction(symbol.getName());
